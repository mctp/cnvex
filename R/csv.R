.geneCSV <- function(cnv, opts) {
    ...
    ## segment
    tmp.seg <- tile.data[,.(seg, lr, baf)]
    tmp.seg <- tmp.seg[,.(.N, lr.seg=mean(lr, na.rm=TRUE), mzd.seg=mean(baf, na.rm=TRUE)),
                       by=seg]
    ...
    tmp.loc <- cbind(tmp.loc,
                     tile.data[tmp.loc$subjectHits, .(lr, baf)]
                     )
    tmp.loc <- tmp.loc[,.(n.loc=.N, lr.loc=mean(lr, na.rm=TRUE), mzd.loc=mean(baf, na.rm=TRUE)),
                       by=.(queryHits)]
